...
public:
    ...
    CANDevice(const char* interface, unsigned int readId, std::function<void(const struct can_frame&)> callback) : readId_(readId), writeId_(0), callback_(callback), stopThread_(false), canRead_(true), canWrite_(false) {
        initSocket(interface);
    }

    // Constructor for write-only CAN device
    CANDevice(const char* interface, unsigned int writeId) : readId_(0), writeId_(writeId), callback_(nullptr), stopThread_(false), canRead_(false), canWrite_(true) {
        initSocket(interface);
    }

    // Constructor for read-write CAN device
    CANDevice(const char* interface, unsigned int readId, std::function<void(const struct can_frame&)> callback, unsigned int writeId) : readId_(readId), writeId_(writeId), callback_(callback), stopThread_(false), canRead_(true), canWrite_(true) {
        initSocket(interface);
    }
    ...
    void initSocket(const char* interface) {
